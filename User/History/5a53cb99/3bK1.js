...
for (let i = 0; i < 1; i++) {
    ...
    if (i = 2) {
        ...
        SSRServer.use(express.static(path.resolve(path.dirname(fileURLToPath(import.meta.url)), '/var/www/localhost/htdocs/CloudNine/Production/clientSide'), { index: false }));
        ...
        SSRServer.use('*', async (_, res) => {
            ...
            try {
                const template = fs.readFileSync('./Production/clientSide/index.html', 'utf-8');
