...
export default function HotBuild() {
    ...
    const hmrBuild = async () => {
        bundling = true
        await build({'build': { outDir: './BasePlatForm/CSRServer', ssr: './SSRServer/SSRServer.jsx', outDir: 'BasePlatForm/SSRServer' }}) // <--- you can give a custom config here or remove it to use default options
