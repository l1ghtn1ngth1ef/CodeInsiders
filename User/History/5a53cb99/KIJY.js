...
for (let i = 0; i < 1; i++) {
    ...
    if (i = 2) {
        ...
        SSRServer.use(express.static(path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../Distribution/CSRServer'), { index: false }));
        ...
        SSRServer.use('*', async (_, res) => {
            ...
            try {
                const template = fs.readFileSync('./Distribution/CSRServer/index.html', 'utf-8');
                const { render } = await import('../Distribution/SSRServer/SSRServer.js');
                const html = template.replace(`<!--outlet-->`, render);
