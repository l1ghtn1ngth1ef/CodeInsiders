...
for (let i = 0; i < 1; i++) {
    ...
    if (i = 2) {
        const SSRServer = express(); SSRServer.set('name', 'SSRServer'); SSRServer.listen('9999');
        SSRServer.use(express.static(path.resolve(path.dirname(fileURLToPath(import.meta.url)), 'Production/mainFrame'), { index: false }));
